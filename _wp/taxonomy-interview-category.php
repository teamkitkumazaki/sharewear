...
<body>
  ...
  <article id="interviewList" class="page-interview-list">
        ...
        <div class="inner_box">
          <div class="category_wrapper">
            <div class="comp-category-list">
              <div class="category_item">
                <a href="/recruit/interview"><span class=>#</span>すべて</a>
              </div>
              <?php
              // カスタム分類名
              $taxonomy = 'interview-category';
              ...
              if ( count( $terms ) != 0 ) {
                foreach ( $terms as $term ) {
                  ...
                  if($term_slug == $current){
    								echo '<div class="category_item"><a href="'.esc_url( $term_link ).'" class="active"><span>#</span>'.$term_name.'</a></div>';
    							}else{
          ...
          <div class="comp-pager">
            <div class="comp_inner">
              <?php if ($paged != 1){
                echo '<a class="link previous" href="'.site_url().'/interview-category/'.$title_en.'/page/'.$previous_paged.'/"><span>前へ</span></a>';
              }else{
                echo '<a class="link previous disabled" style="pointer-events: none; opacity: 0.5; background: #f8f8f8;" href="'.site_url().'/interview-category/'.$title_en.'/page/'.$previous_paged.'/"><span>前へ</span></a>';
              };?>
              <div class="pager_select">
                <select class="pjax-select" name="pager" onchange="location.href=value;">
                  <?php for ($i = 1; $i <= $pager_num; $i++) {
                    if ($i == $paged){
                      echo '<option value="'.site_url().'/interview-category/'.$title_en.'/page/'.$i.'/" selected="selected">'.$i.'/'.intval($pager_num).'</option>';
                    }else{
                      echo '<option value="'.site_url().'/interview-category/'.$title_en.'/page/'.$i.'/">'.$i.'/'.intval($pager_num).'</option>';
                    }
                  };?>
                </select>
              ...
              <?php if ($paged != intval($pager_num)){
                echo '<a class="link next" href="'.site_url().'/interview-category/'.$title_en.'/page/'.$next_paged.'/"><span>次へ</span></a>';
              }else{
                echo '<a class="link next disabled" style="pointer-events: none; opacity: 0.5; background: #f8f8f8;" href="'.site_url().'/interview-category/'.$title_en.'/page/'.$next_paged.'/"><span>次へ</span></a>';
              };?>
            </div><!-- comp_inner -->
          </div><!-- comp-pager -->
          <?php endif; ?>
